...
function check_for_ruby {
  ...
  if [[ $ruby_output = "" ]];then
    echo "Ruby needed for installation, please install Ruby (1.8.x) before installing DTK CLI. Example: sudo apt-get install ruby1.8"
    exit 0
  fi
}
...
function check_for_ruby_gems {
  ...
  if [[ $gem_output = "" ]];then
    echo "Ruby gems are needed for installation, please install rubygems before installing DTK CLI. Example: sudo apt-get install rubygems"
    exit 0
  fi
}
